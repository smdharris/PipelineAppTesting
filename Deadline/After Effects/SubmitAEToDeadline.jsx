{
	...
	function callDeadlineCommand( args )
	{
		...
		if (system.osName == "MacOS")
		{
			var deadlinePathFile = getFileHandle( "/Users/Shared/Thinkbox/DEADLINE_PATH", true);
	...
	function SubmitToDeadline( projectName, projectPath, version )
	{
		...
		var initDraftQuick = parseBool( getIniSetting( "DraftQuick", "true" ) );
		var initDraftFormat = getIniSetting( "DraftFormat", "" );
		var initDraftCodec = getIniSetting( "DraftCodec", "" );
		var initDraftResolution = getIniSetting( "DraftResolution", "");
		var initDraftQuality = parseInt( getIniSetting( "DraftQuality", 85 ) );
		var initDraftFrameRate = getIniSetting( "DraftFrameRate", "" );
		...
		dialog.includeOutputPath = dialog.failOnExistingProcessGroup.add( 'checkbox', undefined, 'Include Output File Path' );
		dialog.includeOutputPath.value = initIncludeOutputPath;
		dialog.includeOutputPath.size = checkBoxCSize;
		...
		{
			// Integration tab
			dialog.integrationTab = dialog.tabPanel.add( 'tab', undefined, 'Integration' );
		}
		else
		{
			// Place the panel at [0,0,0,0] and then hide it.
			dialog.integrationTab = dialog.add( 'panel', [0,0,0,0] );
			dialog.integrationTab.visible = false;
		}
		...
		function getProjMgmtInfo( scriptPath, additionalArgs )
		{
			var keyValuePairs = {};
			var args = '-ExecuteScript "' + scriptPath + '" AfterEffects ' +additionalArgs;
			var output = callDeadlineCommand( args );
			...
			if (outLines.length > 1)
			{
				...
				{
					if ( failregex.exec(outLines[ i ] ) == null )
					{
		...
		function updateProjMgmtUI( forceOn )
		{
			...
			var draftUploadFilmStripEnabled = false;
			if ( projectManager == "Shotgun" && shotgunKVPs && shotgunKVPs['UserName'] )
			{
				...
				draftUploadMovieEnabled = createValue;
				draftUploadFilmStripEnabled = createValue;
			...
			dialog.draftCreateFilmStrip.enabled = draftUploadFilmStripEnabled;
			...
			dialog.draftQuick.enabled = dialog.useDraft.value;
		}
		
		//Connect/Use Shotgun
		dialog.projectMgmtPanel = dialog.integrationTab.add( 'panel', undefined, 'Project Management Integration' );
		...
		dialog.projectMgmtCombo.onChange = function()
		{
			...
			if ( selection == "FTrack" )
			{
				...
				draftUseValuesLabel = "Use FTrack Data";
				draftUseValuesAnn = "Uses data from FTrack to fill out the Draft settings";
				draftUploadlabel = "Upload Draft Results To FTrack";
				draftAnnotation = "Check to upload Draft output to FTrack.";
			...
			dialog.draftCreateMovie.enabled = draftCreateMovieEnabled;
			dialog.draftCreateFilmStrip.enabled = draftCreateFilmStripEnabled;
			...
			dialog.createVersion.text = createLabel;
		...
		dialog.projectMgmtConnectButton.onClick = function()
		{
			...
			if ( projectManager == "Shotgun" )
			{
				...
				if ( shotgunKVPs['UserName'] )
				{
					args = args + "UserName="+shotgunKVPs['UserName'] + " ";
				}
				
				if ( shotgunKVPs['VersionName'] )
					args = args + "VersionName="+ shotgunKVPs['VersionName']+ " ";
				
				if ( shotgunKVPs['TaskName'] )
				{
					args = args + "TaskName="+ shotgunKVPs['TaskName']+ " ";
				}
				if ( shotgunKVPs['ProjectName'] )
				{
					args = args + "ProjectName="+ shotgunKVPs['ProjectName']+ " ";
				}
				if( shotgunKVPs['EntityType'] )
				{
					args = args + "EntityType="+ shotgunKVPs['EntityType']+ " ";
				}
				if( shotgunKVPs['SequenceName'] )
				{
					args = args + "SequenceName="+ shotgunKVPs['SequenceName']+ " ";
				}
				
				result = getProjMgmtInfo( sgPath, args );
			...
			else if ( projectManager == "FTrack" )
			{
				...
				if ( ftrackKVPs['FT_AssetName'] )
					args = args + "AssetName=" + ftrackKVPs['FT_AssetName']+ " ";

				if ( ftrackKVPs['FT_Username'] )
				{
					args += "UserName=" + ftrackKVPs[ 'FT_Username' ] + " ";
				}
				if ( ftrackKVPs['FT_TaskName'] )
					args += "TaskName=" + ftrackKVPs[ 'FT_TaskName' ] + " ";
				if ( ftrackKVPs['FT_ProjectName'] )
					args += "ProjectName=" + ftrackKVPs[ 'FT_ProjectName' ] + " ";
				
				result = getProjMgmtInfo( ftPath, args );
				if ( result )
					ftrackKVPs = result;
					updateProjMgmtUI( true );
			}
			else if ( projectManager == "NIM" )
			{
				...
				for (var i in nimKVPs)
				{
					args += i+"=\""+ nimKVPs[i]+"\" ";
				}
				
				result = getProjMgmtInfo( nimPath, args );
				if ( result )
					nimKVPs = result;
					updateProjMgmtUI( true );
			}
		...
		function ReadInFormatsFile(filename)
		{
			var results = [];
			
			var formatFile = getFileHandle(filename, false);
			if (formatFile.exists)
			{
				formatFile.open('r');
				fileText = formatFile.read();
				formatFile.close();
				...
				for( var i = 0; i < lines.length; i ++ )
				{
					...
					var name = words[1].trim() + " (" + words[0].trim() + ")";
					results.push(name);
					
					FormatsDict[name] = [ words[0].trim(), words[2].trim() ];
				}
			}
		...
		function ReadInCodecsFile(filename)
		{
			var formatFile = File(filename);
			if (formatFile.exists)
			{
				formatFile.open('r');
				fileText = formatFile.read();
				...
				for( var i = 0; i < lines.length; i ++ )
				{
					...
					var words = line.split(":");
					var format = words[0].trim();
					var codecs = words[1];
					var codecList = codecs.split(",");

					for(var j = 0; j < codecList.length; j++)
						codecList[j] = codecList[j].trim();

					if( !(format in CodecsDict) )
						CodecsDict[format] = {};

					CodecsDict[format] = codecList;
				}
			}
		}

		function ReadInResolutionsFile(filename)
		{
			var results = [];
			...
			if (formatFile.exists)
			{
				...
				for( var i = 0; i < lines.length; i ++ )
				{
					...
					ResolutionsDict[name] = words[0].trim();
				}
			}
			return results;
		}
		...
		function ReadInRestrictionsFile(filename)
		{
			var results = [];
			...
			if (formatFile.exists)
			{
				...
				for( var i = 0; i < lines.length; i ++ )
				{
					...
					var restriction = words[1].split("=");
					var restrictionType = restriction[0].trim();
					var restrictionList = restriction[1].split(",");
					
					for(var j = 0; j < restrictionList.length; j++)
						restrictionList[j] = restrictionList[j].trim();
						
					if( !(name in RestrictionsDict) )
					{
						results.push(name);
		...
		function ReadInDraftOptions()
		{
			var mainDraftFolder = root;
			var formatFilename = mainDraftFolder;
			var codecFilename = mainDraftFolder;
			var resolutionFilename = mainDraftFolder;
			var frameRateFilename = mainDraftFolder;
			var restrictionFilename = mainDraftFolder;
			
			if (system.osName == "MacOS")
			{
				mainDraftFolder = root + "/submission/Draft/Main";
				formatFilename = mainDraftFolder + "/formats.txt";
				codecFilename = mainDraftFolder + "/codecs.txt";
				resolutionFilename = mainDraftFolder + "/resolutions.txt";
				frameRateFilename = mainDraftFolder + "/frameRates.txt";
				restrictionFilename = mainDraftFolder + "/restrictions.txt";
			}
			else
			{
				mainDraftFolder = root + "\\submission\\Draft\\Main";
			...
			Formats = ReadInFormatsFile( formatFilename );
			ReadInCodecsFile( codecFilename );
			Resolutions = ReadInResolutionsFile( resolutionFilename );
			FrameRates = ReadInFile( frameRateFilename );
			Restrictions = ReadInRestrictionsFile( restrictionFilename );
			
		}
		
		function SetOptions( selectionType, options)
		{
			var draftBox = null;
			var selection = null;
			if(selectionType == "Codec")
			{
				draftBox = dialog.draftCodec;
				selection = String(dialog.draftCodec.selection);
			}
			else
			{
				draftBox = dialog.draftFrameRate;
				selection = String(dialog.draftFrameRate.selection);
			}
			...
			if(selectionIndex != -1)
			{
				draftBox.selection = selectionIndex;
			}
			else if(selectionType == "FrameRate" && twoFourIndex != -1)
				draftBox.selection = twoFourIndex;
			else
				draftBox.selection = 0
		
		}
		
		function GetOptions(selection, selectionType, validOptions)
		{
			...
			if( selection in RestrictionsDict)
			{
				...
				{
					var restrictedOptions = RestrictionsDict[selection][selectionType];
					for(var i = 0; i < restrictedOptions.length; i++)
					{
						var found = 0;
						for(var j = 0; j < validOptions.length; j++)
						{
							if( validOptions[j] == restrictedOptions[i] )
							{
								found = 1;
							}
						}
						...
						if(found != 0 )
						{
							intersectionOptions.push(restrictedOptions[i]);
						}
		...
		function AdjustCodecs()
		{
			var selectedFormat = FormatsDict[dialog.draftFormat.selection][0];
			var validOptions = CodecsDict[selectedFormat];
			validOptions = GetOptions(selectedFormat, "Codec", validOptions);
			
			SetOptions("Codec", validOptions);
		}
		
		function AdjustFrameRates()
		{
			var validOptions = FrameRates;
			
			var selectedFormat = FormatsDict[dialog.draftFormat.selection][0];
			validOptions = GetOptions(selectedFormat, "FrameRate", validOptions);
			
			var selectedCodec = dialog.draftCodec.selection;
		...
		{
			var quickEnabled = dialog.draftQuick.value;
			var selectedFormat = FormatsDict[dialog.draftFormat.selection][0];
			var selectedCodec = dialog.draftCodec.selection.text;
			var enableQuality = ValidQuality( selectedFormat, selectedCodec, "EnableQuality" );
			dialog.draftQualityGroup.enabled = quickEnabled && dialog.useDraft.value && enableQuality;
		}

		function ValidQuality( selectedFormat, selectedCodec, enableQuality )
		{
			if( selectedFormat in RestrictionsDict )
			{
				if( enableQuality in RestrictionsDict[selectedFormat] )
				{
					var codecsList = RestrictionsDict[selectedFormat][enableQuality];
					for( var i = 0; i < codecsList.length; i++ )
					{
						if( codecsList[i] == selectedCodec )
							return true;
					}
				}
			}
		...
		function IsMovieFromFormat( format )
		{
			if(format in FormatsDict)
			{
		...
		function EnableDraftFields(quick)
		{
			dialog.draftQuickGroup.enabled = dialog.useDraft.value;
			
			var quickEnabled = dialog.draftQuick.value;
			var draftCreateMovie = IsMovieFromFormat(dialog.draftFormat.selection);
			
			dialog.draftFormatGroup.enabled = quickEnabled && dialog.useDraft.value;
			dialog.draftResolutionGroup.enabled = quickEnabled && dialog.useDraft.value;
			dialog.draftCodecLabel.enabled = dialog.draftFormatGroup.enabled;
			dialog.draftCodec.enabled = dialog.draftFormatGroup.enabled;
			dialog.draftFrameRateLabel.enabled = dialog.draftResolutionGroup.enabled && draftCreateMovie;
			dialog.draftFrameRate.enabled = dialog.draftResolutionGroup.enabled && draftCreateMovie;
			
			dialog.draftTemplateGroup.enabled = !quickEnabled && dialog.useDraft.value;
			dialog.draftUserGroup.enabled = !quickEnabled && dialog.useDraft.value;
			dialog.draftEntityGroup.enabled = !quickEnabled && dialog.useDraft.value;
			dialog.draftVersionGroup.enabled = !quickEnabled && dialog.useDraft.value;
			dialog.draftExtraArgsGroup.enabled = !quickEnabled && dialog.useDraft.value;
			
			dialog.draftUseSGDataButton.enabled = dialog.useDraft.value && dialog.createVersion.value;
			dialog.uploadDraftToShotgun.enabled = dialog.useDraft.value && dialog.createVersion.value;

			AdjustQuality();
		}
		
		ReadInDraftOptions();
		dialog.draftPanel = dialog.integrationTab.add( 'panel', undefined, 'Draft Options' );
		...
		dialog.draftUseSGDataButton.onClick = function()
		{
			...
			{
				if ( ftrackKVPs['FT_Username'] )
					dialog.draftUserText.text = ftrackKVPs['FT_Username'];
				
				if ( ftrackKVPs['FT_AssetName'] )
					dialog.draftVersionText.text = ftrackKVPs['FT_AssetName'];
				
				if ( ftrackKVPs['FT_TaskName'] )
					dialog.draftEntityText.text = ftrackKVPs['FT_TaskName'];
			}
			else if ( dialog.projectMgmtCombo.selection.toString() == "NIM" )
			{
				...
				if( nimKVPs['nim_taskID'] )
				{
					var task = nimKVPs['nim_taskID'];
					if( task.length > 0 )
						entity = task;
				}
				
				if( entity.length == 0 && nimKVPs['nim_assetName'] )
				{
					var asset = nimKVPs['nim_assetName'];
					if( asset.length > 0 )
						entity = asset;
				}
				
				if( entity.length == 0 && nimKVPs['nim_shotName'] )
				{
					var shotName = nimKVPs['nim_shotName'];
					if( shotName.length > 0 )
						entity = shotName;
				}
					
				dialog.draftEntityText.text = entity;
			}
		}
		dialog.draftUseSGDataButton.enabled = false;
		
		dialog.draftQuickGroup = dialog.draftPanel.add( 'group', undefined );
		dialog.draftQuickGroup.alignment = [ScriptUI.Alignment.FILL, ScriptUI.Alignment.TOP];
		dialog.draftQuick = dialog.draftQuickGroup.add( 'checkbox', undefined, 'Quick' );
		dialog.draftQuick.value = initDraftQuick;
		dialog.draftQuick.onClick = function()
		{
			EnableDraftFields(1);
		}
		
		dialog.draftFormatGroup = dialog.draftPanel.add( 'group', undefined );
		dialog.draftFormatGroup.alignment = [ScriptUI.Alignment.FILL, ScriptUI.Alignment.TOP];
		dialog.draftFormatLabel = dialog.draftFormatGroup.add( 'statictext', undefined, 'Format' );
		dialog.draftFormatLabel.size = labelSize;
		dialog.draftFormat = dialog.draftFormatGroup.add( 'dropdownlist', undefined );
		dialog.draftFormat.size = comboSize;
		dialog.draftFormat.onChange = function()
		{
			EnableDraftFields(0);
			AdjustCodecs();
		}
		...
		{
			if( Formats[i] == initDraftFormat )
				selectedIndex = i;
				
			dialog.draftFormat.add( 'item', Formats[i] );
		}
		
		if( selectedIndex >= 0 )
			dialog.draftFormat.selection = dialog.draftFormat.items[selectedIndex];
		else if( dialog.draftFormat.items.length > 0 )
			dialog.draftFormat.selection = dialog.draftFormat.items[0]
		
		dialog.draftCodecLabel = dialog.draftFormatGroup.add( 'statictext', undefined, 'Compression' );
		dialog.draftCodecLabel.size = labelSize;
		dialog.draftCodec = dialog.draftFormatGroup.add( 'dropdownlist', undefined );
		dialog.draftCodec.size = comboSize;
		dialog.draftCodec.onChange = function()
		{
			AdjustFrameRates();
			AdjustQuality();
		}
		...
		selectedIndex = -1;
		var selectedFormat = FormatsDict[dialog.draftFormat.selection][0];
		var codecList = CodecsDict[selectedFormat];

		for(var i = 0; i < codecList.length; i++)
		{
			if( codecList[i] == initDraftCodec )
				selectedIndex = i;
				
			dialog.draftCodec.add( 'item', codecList[i] );
		}
		
		if( selectedIndex >= 0 )
			dialog.draftCodec.selection = dialog.draftCodec.items[selectedIndex];
		else if( dialog.draftCodec.items.length > 0 )
			dialog.draftCodec.selection = dialog.draftCodec.items[0]
			
		dialog.draftFormatGroup.enabled = dialog.draftQuick.value
		
		dialog.draftResolutionGroup = dialog.draftPanel.add( 'group', undefined );
		dialog.draftResolutionGroup.alignment = [ScriptUI.Alignment.FILL, ScriptUI.Alignment.TOP];
		dialog.draftResolutionLabel = dialog.draftResolutionGroup.add( 'statictext', undefined, 'Resolution' );
		dialog.draftResolutionLabel.size = labelSize;
		dialog.draftResolution = dialog.draftResolutionGroup.add( 'dropdownlist', undefined );
		dialog.draftResolution.size = comboSize;
		
		selectedIndex = -1;
		
		for(var i = 0; i < Resolutions.length; i++)
		{
			if( Resolutions[i] == initDraftResolution )
				selectedIndex = i;
				
			dialog.draftResolution.add( 'item', Resolutions[i] );
		}
		
		if( selectedIndex >= 0 )
			dialog.draftResolution.selection = dialog.draftResolution.items[selectedIndex];
		else if( dialog.draftResolution.items.length > 0 )
			dialog.draftResolution.selection = dialog.draftResolution.items[0]
		
		dialog.draftFrameRateLabel = dialog.draftResolutionGroup.add( 'statictext', undefined, 'Frame Rate' );
		dialog.draftFrameRateLabel.size = labelSize;
		dialog.draftFrameRate = dialog.draftResolutionGroup.add( 'dropdownlist', undefined );
		dialog.draftFrameRate.size = comboSize;
		
		selectedIndex = -1;
		
		for(var i = 0; i < FrameRates.length; i++)
		{
			if( FrameRates[i] == initDraftFrameRate )
				selectedIndex = i;
				
			dialog.draftFrameRate.add( 'item', FrameRates[i] );
		}
		
		if( selectedIndex >= 0 )
			dialog.draftFrameRate.selection = dialog.draftFrameRate.items[selectedIndex];
		else if( dialog.draftFrameRate.items.length > 0 )
			dialog.draftFrameRate.selection = dialog.draftFrameRate.items[0]
			
		dialog.draftResolutionGroup.enabled = dialog.draftQuick.value
		
		dialog.draftQualityGroup = dialog.draftPanel.add( 'group', undefined );
		dialog.draftQualityGroup.alignment = [ScriptUI.Alignment.FILL, ScriptUI.Alignment.TOP];
		dialog.draftQualityLabel = dialog.draftQualityGroup.add('statictext', undefined, 'Quality');
		dialog.draftQualityLabel.size = labelSize;
		dialog.draftQuality = dialog.draftQualityGroup.add( 'edittext', undefined, initDraftQuality );
		dialog.draftQuality.size = shortTextSize;
		dialog.draftQuality.onChange = function()
		{
			setSliderValue( this.text, 0, 100, dialog.draftQualitySlider )
			this.text = Math.round( dialog.draftQualitySlider.value );
		}
		
		dialog.draftQualitySlider = dialog.draftQualityGroup.add( 'slider', undefined, initDraftQuality, 0, 100);
		dialog.draftQualitySlider.onChange = function() { dialog.draftQuality.text = Math.round( this.value ); }
		dialog.draftQualitySlider.size = sliderSize;
		
		dialog.draftQualityGroup.enabled = dialog.draftQuick.value
		...
		dialog.submitButton.onClick = function()
		{
			...
			if ( queuedCount != 0 )
			{
				...
				if( checkForDuplicateComps() )
					errors += "At least 2 of your items in the Render Queue have the same name. Please ensure that all of your items have unique names.\n";
			...
			{
				alert( "You do not have any items set to render in the render queue!" );
			}
		}
		...
		dialog.closeButton.onClick = function()
		{
			...
			setIniSetting( "DraftQuick", toBooleanString( dialog.draftQuick.value ) );
			setIniSetting( "DraftFormat", dialog.draftFormat.selection.toString() );
			setIniSetting( "DraftCodec", dialog.draftCodec.selection.toString() );
			setIniSetting( "DraftResolution", dialog.draftResolution.selection.toString() );
			setIniSetting( "DraftQuality", Math.round( dialog.draftQualitySlider.value ) );
			setIniSetting( "DraftFrameRate", dialog.draftFrameRate.selection.toString() );
	...
	function SubmitComp( projectPath, renderQueueItem, layers, jobName, previousJobId, projMgmtKVPs )
	{
		...
		if ( dialog.createVersion.value && dialog.createVersion.enabled )
		{
			...
			else if ( dialog.projectMgmtCombo.selection.toString() == "FTrack" )
			{
				...
				if(dialog.draftCreateMovie.value)
				{
					submitInfoFile.writeln("ExtraInfoKeyValue"+nextKVP+"=Draft_CreateFTMovie=True");
					nextKVP += 1;
					groupBatch = true
				}
				if (groupBatch && !dependentComps)
			...
			{
				submitInfoFile.writeln( "ExtraInfo0=" + projMgmtKVPs['nim_renderName'] );
				submitInfoFile.writeln( "ExtraInfo1=" + projMgmtKVPs['nim_jobName'] );
				submitInfoFile.writeln( "ExtraInfo2=" + projMgmtKVPs['nim_showName'] );
				submitInfoFile.writeln( "ExtraInfo3=" + projMgmtKVPs['nim_shotName'] );
				submitInfoFile.writeln( "ExtraInfo4=" + projMgmtKVPs['nim_description'] );
				submitInfoFile.writeln( "ExtraInfo5=" + projMgmtKVPs['nim_user'] );
				...
				if(dialog.draftCreateMovie.value)
				{
					submitInfoFile.writeln("ExtraInfoKeyValue"+nextKVP+"=Draft_CreateNimMovie=True");
					nextKVP += 1;
					groupBatch = true
				}
				if (groupBatch && !dependentComps)
		...
		{
			if( dialog.draftQuick.value)
			{
				submitInfoFile.writeln( "ExtraInfoKeyValue" + nextKVP + "=SubmitQuickDraft=True" )
				nextKVP += 1;
				submitInfoFile.writeln( "ExtraInfoKeyValue" + nextKVP + "=DraftExtension=" + FormatsDict[dialog.draftFormat.selection.toString()][0] )
				nextKVP += 1;
				submitInfoFile.writeln( "ExtraInfoKeyValue" + nextKVP + "=DraftType=" + FormatsDict[dialog.draftFormat.selection.toString()][1] )
				nextKVP += 1;
				submitInfoFile.writeln( "ExtraInfoKeyValue" + nextKVP + "=DraftResolution=" + ResolutionsDict[dialog.draftResolution.selection.toString()] )
				nextKVP += 1;
				submitInfoFile.writeln( "ExtraInfoKeyValue" + nextKVP + "=DraftCodec=" + dialog.draftCodec.selection.toString() )
				nextKVP += 1;
				submitInfoFile.writeln( "ExtraInfoKeyValue" + nextKVP + "=DraftQuality=" + Math.round(dialog.draftQualitySlider.value) )
				nextKVP += 1;
				submitInfoFile.writeln( "ExtraInfoKeyValue" + nextKVP + "=DraftFrameRate=" + dialog.draftFrameRate.selection.toString() )
				nextKVP += 1;
			}
			else
			{
			...
			{
				submitInfoFile.writeln( "ExtraInfoKeyValue" + nextKVP + "=FT_DraftUploadMovie=" + toBooleanString(dialog.uploadDraftToShotgun.value) )
				nextKVP += 1;
			}
			else if ( dialog.projectMgmtCombo.selection.toString() == "NIM" )
			{
